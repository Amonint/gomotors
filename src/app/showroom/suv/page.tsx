...
const SUVPage = () => {
  ...
  // Obtener marcas únicas para SUVs
  const brands = [...new Set(suvVehicles.map(vehicle => vehicle.brand))];
  const types = ['SUV']; // Solo mostramos SUVs en esta página
  
  useEffect(() => {
    // Filtrar vehículos cuando cambien los filtros
    let result = [...suvVehicles];
        ...
        <div className="mb-8">
          ...
          >
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              SUVs & <span className="text-[#ffe600]">Crossovers</span>
            </h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              Vehículos versátiles con mayor altura al piso, espacio interior y capacidad para todo tipo de terrenos.
            </p>
          </motion.div>
        </div>
            ...
            {filteredVehicles.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 px-4 bg-[#111111] rounded-xl">
                ...
                <p className="text-gray-400 text-center max-w-md">
                  No hay SUVs que coincidan con los filtros seleccionados. Intenta modificar tus criterios de búsqueda.
                </p>
              </div>
            )}
          </div>

...
import { FaSearch, FaDownload, FaEye, FaFilter, FaCalendarAlt } from "react-icons/fa";
...
const RecoleccionAdminPage = () => {
  const [recolecciones, setRecolecciones] = useState<RecoleccionData[]>([]);
  ...
  const [filtroRazon, setFiltroRazon] = useState("todos");
  ...
  const fetchRecolecciones = async (page = 1) => {
    setLoading(true);
    setError("");
    ...
    try {
      const params = new URLSearchParams({
        ...
        limit: "20",
        ...(search && { search }),
        ...(filtroAsesor !== "todos" && { asesor: filtroAsesor }),
        ...(filtroRazon !== "todos" && { razon: filtroRazon }),
        ...(fechaDesde && { fechaDesde }),
        ...(fechaHasta && { fechaHasta })
      });

      const response = await fetch(`/api/recoleccion-datos?${params}`, {
        headers: {
  ...
  const handleSearch = () => {
    fetchRecolecciones(1);
  };

  const handlePageChange = (newPage: number) => {
    fetchRecolecciones(newPage);
  };

  const exportToExcel = async () => {
    setExportando(true);
    try {
      const response = await fetch('/api/recoleccion-datos/export', {
        headers: {
          'Authorization': `Basic ${btoa('admin:lo')}`
        }
      });
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `Recoleccion_Datos_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-neutral-600 mt-1">
              Gestión de formularios de recolección de datos de clientes
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-black focus:border-transparent text-black bg-white"
            >
              <option value="todos">Todas las razones</option>
              {razones.map((razon) => (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-medium text-neutral-900">Total de Registros</h3>
                  ...
                  {recolecciones.map((registro) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <Link
                          href={`/admin/recoleccion/${registro.id}`}
...
export default RecoleccionAdminPage;